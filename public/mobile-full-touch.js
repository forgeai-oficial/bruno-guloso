(()=>{
  ...
  function releaseAll(){jumpPointers.clear();forwardPointers.clear();setJump(false);setDir(0)}

  surface.addEventListener("pointerdown",e=>{
    if(e.pointerType==="mouse"||!active())return;
    if(isForwardBlock(e.clientX,e.clientY)){
      forwardPointers.add(e.pointerId);setDir(1);vibrate(6);
      try{surface.setPointerCapture(e.pointerId)}catch(err){}
      e.preventDefault();return;
    }
    jumpPointers.add(e.pointerId);setJump(true);vibrate(7);
    try{surface.setPointerCapture(e.pointerId)}catch(err){}
    e.preventDefault();
  },{passive:false});
  ...
  surface.addEventListener("touchstart",e=>{
    ...
    for(const t of Array.from(e.changedTouches||[])){
      if(isForwardBlock(t.clientX,t.clientY)){forwardPointers.add("t"+t.identifier);setDir(1)}
      else{jumpPointers.add("t"+t.identifier);setJump(true)}
    }
    e.preventDefault();
  },{passive:false});
  ...
  surface.addEventListener("touchend",e=>{
    ...
    for(const t of Array.from(e.changedTouches||[])){
      forwardPointers.delete("t"+t.identifier);jumpPointers.delete("t"+t.identifier);
    }
    releaseForward();releaseJump();e.preventDefault();
  },{passive:false});
  ...
  window.__bgMobileFullTouch={get enabled(){return true},get mode(){return "tap-free-area-jump-forward-block-move"}};
})();
