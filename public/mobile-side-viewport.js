(()=>{
  ...
  function logicalWidth(){
    let vh=Math.max(1,window.visualViewport?window.visualViewport.height:innerHeight);
    let vw=Math.max(1,window.visualViewport?window.visualViewport.width:innerWidth);
    if(window.__bgLandscapeSessionActive===true&&vw<vh){const t=vw;vw=vh;vh=t;}
    return Math.max(320,Math.min(560,Math.round(240*vw/vh)));
  }

  function ensureSize(w){
    if(w===lastW)return;
    lastW=w;wide.width=w;wide.height=240;ctx.imageSmoothingEnabled=false;
  }
  ...
  function drawSprites(st,cam,layer){
    ...
    for(let i=0;i<st.Sprites.Objects.length;i++){
      ...
      if(s&&s.Layer===layer&&typeof s.Draw==="function"){try{s.Draw(ctx,cam)}catch(e){}}
    }
    ctx.restore();
  }
  ...
  function render(){
    ...
    const st=getState();
    if(!st||!st.Level||!st.Camera||!st.Layer||!st.BgLayer){requestAnimationFrame(render);return;}

    const w=logicalWidth();ensureSize(w);ctx.clearRect(0,0,w,240);
    const side=(w-320)/2;
    ...
    try{
      ...
      for(let i=0;i<st.BgLayer.length;i++){
        if(st.BgLayer[i]){oldBgW[i]=st.BgLayer[i].Width;st.BgLayer[i].Width=w;st.BgLayer[i].Draw(ctx,cam);}
      }
  ...
  requestAnimationFrame(render);
  window.__bgMobileSideViewport={get enabled(){return true},get logicalWidth(){return lastW||logicalWidth()}};
})();
