(()=>{
  ...
  if(window.__BG_TEST_MECHANICS_RUNTIME_V2__)return;
  window.__BG_TEST_MECHANICS_RUNTIME_V2__=true;
  ...
  function syncDonuts(state){
    try{
      const l=state&&state.Level,d=state&&state.__bgCollectibleData;
      if(!l||!d||!Array.isArray(d.items))return;
      const src=[].concat(Array.isArray(l.__bgV5Donuts)?l.__bgV5Donuts:[],Array.isArray(l.__bgAllTestDonuts)?l.__bgAllTestDonuts:[]);
      const have=new Set(d.items.map(i=>i&&(i.__bgV5Key||i.__bgAllTestKey)).filter(Boolean));
      for(const q of src){const key=q&&q.key;if(!key||have.has(key))continue;d.items.push({x:q.x,y:q.y,taken:false,phase:q.phase||0,__bgV5Key:key,__bgAllTestKey:key});have.add(key)}
    }catch(e){}
  }

  function installSpawn(){
    if(!(window.Mario&&Mario.SpriteTemplate&&Mario.SpriteTemplate.prototype))return false;
    const p=Mario.SpriteTemplate.prototype;if(p.__bgV5RuntimeSpawnV2)return true;
    const old=p.Spawn;p.__bgV5RuntimeSpawnV2=true;
    p.Spawn=function(world,b,c,facing){
      const k=this.__bgV5Kind||this.__bgTestVariant;
      if(!["moveH","elevator","fall","osc","spring"].includes(k))return old.apply(this,arguments);
      if(this.IsDead)return;
      const x=b*16+8,y=(c+1)*16-1;let s=null;
      const range=this.__bgRange||this.range||30,phase=this.__bgPhase||this.phase||0;
      if(k==="moveH")s=new MovingH(world,x,y,range,phase);
      else if(k==="elevator")s=new Elevator(world,x,y,range||40,phase);
  ...
  function installStateHook(){
    ...
    const p=Mario.LevelState.prototype;if(p.__bgV5RuntimeStateV2)return true;
    const enter=p.Enter,update=p.Update;p.__bgV5RuntimeStateV2=true;
  ...
  window.__bgV5Mechanics={active:true,legacyBuilderDisabled:true};
})();
